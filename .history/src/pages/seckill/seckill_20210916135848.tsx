...
const Seckill = () => {
    let list = useSelector((state: any) => state.seckill.topics)
    ...
    let btn = (item: any) => {
        ...
        setestimate(2)
        setId(item._id)
        form.setFieldsValue({ goods_number: item.goods_number })
        form.setFieldsValue({ price: item.price })
        form.setFieldsValue({ goods: item.goods._id })
        form.setFieldsValue({ end_time: moment(item.end_time) })
        form.setFieldsValue({ start_time: moment(item.start_time) })
    }
        ...
        <div style={{ backgroundColor: '#fff', width: '100%', height: '100%', padding: 20 }}>
            ...
            <div>
                <Table dataSource={list} rowKey='_id'>
                    <Column title="秒杀商品" dataIndex="goods"
                        key="isShow"
                    ...
                    <Column title="秒杀价格" dataIndex="price" />
                    <Column title="秒杀数量" dataIndex="goods_number" />
                    <Column
                        title="是否禁用 "
                        dataIndex="isShow"
                        key="isShow"
                    ...
                    <Column
                        ...
                        render={(text, record: any) => (
                            <Space size="middle">
                                ...
                                <Button type="primary" danger onClick={() => { reduce(record) }}>删除</Button>
                            </Space>
                        )}
                    />
                </Table>
