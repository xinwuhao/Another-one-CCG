        ...
        <div style={{ width: '100%', backgroundColor: 'white', padding: 10 }}>
            ...
            <div>
                <Table dataSource={topics.data} rowKey='_id'>
                    <Column title="#" width={50} dataIndex="content" />
                    <Column title="商品名称" width={300} dataIndex="name" />
                    <Column title="商品原价" dataIndex="originalPrice" />
                    <Column title="商品现价" dataIndex="presentPrice" />
                    <Column title="商品库存" dataIndex="stock" />
                    <Column title="是否新品" dataIndex="isNewGood"
                        render={(text, record: any) => (
                            <Switch defaultChecked={record.isShow} onChange={() => { Change(record) }} />
                        )} />
                    <Column title="是否热卖" dataIndex="isHot"
                        render={(text, record: any) => (
                            <Switch defaultChecked={record.isShow} onChange={() => { Change(record) }} />
                        )} />
                    <Column title="是否推荐" dataIndex="isRecommend"
                        render={(text, record: any) => (
                            <Switch defaultChecked={record.isShow} onChange={() => { Change(record) }} />
                        )} />
                    <Column
