...
const Recommend = () => {
    ...
    let onSearch = (value: any) => {
        dispatch(({
            ...
            payload: {
                current: current, pageSize: pageSize, query: value
            }
        }))
    };//输入框的值
    // 添加
    ...
    let handleChange = (value: any) => {
        arr = value && value.map((item: any) => {
            return list.data && list.data.filter((item1: any) => {
                return item1._id == item
            })
        })
        setArr(arr)
        // console.log(arr);
            ...
            <div style={{ backgroundColor: '#fff', width: '100%', height: '100%', padding: 20 }}>
                <div>
                    ...
                    <Modal
                        ...
                        destroyOnClose={true}>
                        <div>
                            <Form name="name" onFinish={handleOk} form={form} preserve={false}>
                                <Form.Item
                                    name="name"
                                    label="导航名称">
                                    <Input />
