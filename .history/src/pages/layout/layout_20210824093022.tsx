          ...
          {principal.map((item: any, index: number) => {
            return (
              <div key={index}>
                ...
                <div>{item.name == '商品管理' ?
                  <div>{secondary.map((item1: any, index1: number) => {
                    <div key={index1}>
                      <SubMenu key={`${item.url}`} icon={<item.icon />} title={`${item.name}`}>
                        <Menu.Item key={`${item1.url}`}>{item1.name}</Menu.Item>
                        <Menu.Item key="2">{item1.name}</Menu.Item>
                        <Menu.Item key="3">{item1.name}</Menu.Item>
                        <Menu.Item key="4">{item1.name}</Menu.Item>
                      </SubMenu>
                    </div>
                  })}
                  </div> : <div></div>
