        ...
        <Sider collapsible collapsible={collapsible}>
          ...
          <div>
            {principal.map((item: any, index: number) => {
              return (
                <Menu theme="dark" defaultSelectedKeys={principal[0].url} mode="inline">
                  {item.name !== '商品管理' ? <Menu.Item key={`${item.url}`} icon={<item.icon />}>
                    {item.name}
                  </Menu.Item> : <div></div>}
                  {item.name == '商品管理' ? (
                    <SubMenu key={`${item.url}`} icon={<item.icon />} title={`${item.name}`}>
                      {secondary.map((item1: any, index1: number) => {
                        return (
